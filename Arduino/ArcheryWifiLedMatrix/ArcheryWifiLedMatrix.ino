...
uint8_t _messageBuffer[MAX_MESSAGE_LENGTH + 1];
...
void setup() {
  ...
  for (int i = 0; i < 240; i++){
    showRightAdjNumber(i, 3, _shootNumberHue, 255, _ledIntensity);
    delay(100);
  }
...
void loop() {
  ...
  bool gotNewMessage = receiveByteSerialCom(_messageBuffer);
  if (gotNewMessage) {
  ...
  digitalWrite(POWER_IND_PIN, _t - (unsigned long)_timeOfLastMessage < (unsigned long)500 ? LOW : HIGH);
  _sequence.lastCountdownState = _sequence.countdownState;
}

bool receiveByteSerialCom(uint8_t* buf){
  if (_bytesReceivedCounter == 0){
    _numOfBytesReceived = 0;
  }
  bool messageComplete = false;
  if (_serial1.available()){
    uint8_t c = (uint8_t)_serial1.read();
    ++_bytesReceivedCounter;
    _bytesReceivedCounter = min(MAX_MESSAGE_LENGTH, _bytesReceivedCounter);
    _numOfBytesReceived = _bytesReceivedCounter;
    _messageBuffer[_bytesReceivedCounter - 1] = c;
    _messageBuffer[_bytesReceivedCounter] = '\0';
    messageComplete = (c == '\0' || c == '\n');
    _bytesReceivedCounter = messageComplete ? 0 : _bytesReceivedCounter;
  }
...
void resetToDefault() {
  ...
  _sequence.timeShootingCountdown = _sequence.timeShootingCountdownSeconds * 1000L;
  _sequence.timeShootingCountdownSecondsChangedPulse = true;
  _sequence.timeWaitingCountdownSeconds = 10L;
  _sequence.timeWaitingCountdown = _sequence.timeWaitingCountdownSeconds * 1000L;
...
void updateSound() {
  if (_t > _sound.startTime +  _sound.duration * 10) {
    digitalWrite(SOUND_PIN, OFF);
  } else {
    _sound.sounding = _sound.sounding
                      && (_t - _sound.startTime)
                      < _sound.duration * (unsigned long)(2 * _sound.noSignals - 1);
